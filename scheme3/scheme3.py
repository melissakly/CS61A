...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** REPLACE THIS LINE ***"
        expr_part= scheme_eval(first, env)
        args=rest.map(lambda function: scheme_eval(function, env))
        return scheme_apply(expr_part, args, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, do_lambda_form(vals, env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    for binding in range(len(bindings)):
        formals=Pair(bindings[binding]), formals)
        names=Pair(bi)
    new_env=env.make_child_frame(formals, names)"""
    # END PROBLEM 15
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            expr_part= scheme_optimized_eval(first, env)
            check_procedure(expr_part)
            args=rest.map(lambda function: scheme_optimized_eval(function, env))
            result = scheme_apply(expr_part, args, env)
            # END Extra Credit
    return result
